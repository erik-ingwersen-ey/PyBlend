...
def handle_input_path(input_path: str | Path) -> str:
    ...
    This function checks whether the provided path points to a valid JSON file.
    If the input path is a relative path and the file isn't found in the
    current directory, the function searches up to two levels of parent
    directories to locate the file.

    Parameters
    ----------
    input_path : str or Path
        The input path to check, which should point to a JSON file.

    Returns
    -------
    str
        The resolved absolute path to the JSON file.

    Raises
    ------
    ValueError
        If the input path doesn't point to a JSON file.
    FileNotFoundError
        If the file can't be found after searching the current directory
        and up to two levels of parent directories.

    Examples
    --------
    >>> path = handle_input_path("config.json")
    >>> print(path)
    '/absolute/path/to/config.json'

    >>> path = handle_input_path("/path/to/config.json")
    >>> print(path)
    '/path/to/config.json'
    """
...
def handle_output_path(output_path: str | Path) -> str:
    ...
    JSON file. If the path is a directory or lacks a file suffix, the function
    creates the necessary directories and generates a default JSON filename
    with the current timestamp. If the path specifies a file with a non-JSON
    extension, a `ValueError` is raised.

    Parameters
    ----------
    output_path : str or Path
        The output path where the JSON file should be stored.

    Returns
    -------
    str
        The resolved output path as a string, pointing to a JSON file.

    Raises
    ------
    ValueError
        If the output path specifies a file with a non-JSON extension.
